...
| `SECRET_KEY_BASE`     | ❌       | auto-generated (installer) | Signs session cookies. Auto-generated by installer for persistence. Not required in env — entrypoint generates one if absent. |
| `RAILS_MAX_THREADS`   | ❌       | auto-detected              | Puma thread pool (web + Solid Queue workers). Auto-detected from available RAM, clamped 3–16                                  |
| `SOLID_QUEUE_IN_PUMA` | ❌       | —                          | Set to `true` to run job worker inside Puma (required for single-container deploy)                                            |
...
| `DATABASE_URL`      | \*       | —        | PostgreSQL connection string: `postgres://user:pass@host:5432/dbname`. Auto-set by the PG compose override |
| `POSTGRES_USER`     | \*       | —        | PostgreSQL username (used by PG compose override)                                                          |
| `POSTGRES_PASSWORD` | \*       | —        | PostgreSQL password (used by PG compose override)                                                          |
...
# Fill in DOMAIN, LETSENCRYPT_EMAIL, CF_DNS_API_TOKEN (email vars optional)
# SECRET_KEY_BASE is auto-generated by the entrypoint — set explicitly for persistence across restarts
...
```bash
cp deploy/.env.example deploy/.env
# Fill in DOMAIN, TRAEFIK_NETWORK (email vars optional)
# SECRET_KEY_BASE is auto-generated by the entrypoint — set explicitly for persistence across restarts
...
```bash
cp deploy/.env.example deploy/.env
# Fill in APP_PORT if not using default 80 (email vars optional)
# SECRET_KEY_BASE is auto-generated by the entrypoint — set explicitly for persistence across restarts
...
Add the `-f deploy/docker-compose.pg.yml` override to any manual setup command above:

```bash
# Example: Standalone Traefik with PostgreSQL
docker compose \
  -f deploy/compose/app.yml \
...
When using PostgreSQL, set at minimum:

- `DB_PROVIDER=postgres`
- `POSTGRES_USER` and `POSTGRES_PASSWORD` (auto-constructs `DATABASE_URL`)
- Or set `DATABASE_URL` directly

See [docs/adapter-pattern.md](../docs/adapter-pattern.md) for how the adapter system works.
